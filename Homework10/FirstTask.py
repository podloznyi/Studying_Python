...
lstcntr = int(input('Please enter the number of lists: '))
emptylist = []

for k in range(lstcntr):
    inp = input('Enter the Order #, Name of the book, Amount, Price per book: ').split()
    emptylist.append(inp)

listoftuples = list(map(lambda x: (int(x[0]), int(x[-2]) * float(x[-1]))
if int(x[-2]) * float(x[-1]) >= 100 else (int(x[0]), int(x[-2]) * float(x[-1])
                                          + 10), emptylist))
